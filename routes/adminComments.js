...
router.delete('/comment/:commentId', async (req, res)=>{
    try {
        const validateCommentId = Comment.validate(req.params);
        if(validateCommentId.error) throw {statusCode: 400, message: validateCommentId.error};

        const commentToBeDeleted = await Comment.readById(req.params.commentId);

        const deletedComment = await commentToBeDeleted.delete();

        res.send(JSON.stringify(deletedComment));
    } catch (err) {
        let errorMessage;
        if (!err.statusCode) {
            errorMessage = {
                statusCode: 500,
                message: `Comment doesn't exist + ${err}`
            };
        } else {
            errorMessage = err;
        }
